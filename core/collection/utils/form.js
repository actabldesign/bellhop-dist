 ...
 */
export function syncFormValue(component) {
 ...
 */
export function syncCheckableFormValue(internals, name, checked, value = 'on') {
 ...
 */
export function updateValidity(internals, inputEl, customMessage) {
 ...
 */
export function setCustomValidity(internals, isValid, message = '') {
    if (!internals?.setValidity)
        return;
    if (isValid) {
 ...
 */
export function validateRequired(internals, required, checked, message = 'Please check this box if you want to proceed.') {
    if (!internals?.setValidity)
        return;
    if (required && !checked) {
